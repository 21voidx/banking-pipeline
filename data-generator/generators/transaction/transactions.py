...
class TransactionGenerator:
    ...
    def _build_transaction(
        ...
        txn_at = datetime.combine(txn_date, datetime.min.time()).replace(
            hour=txn_hour, minute=txn_minute, second=txn_second
        )
        ...
        return {
            ...
            "balance_after": None,  # Will be computed if needed
            ...
            "transaction_at": txn_at.strftime("%Y-%m-%d %H:%M:%S.%f"),
            "processed_at": txn_at.strftime("%Y-%m-%d %H:%M:%S.%f") if status == "completed" else None,
        }
